...
static constexpr u64 crcTable[256] = {
...
// String Id literals
constexpr strId operator""_sid(const char* cstr, size_t length) {
	return crc64(cstr, static_cast<u32>(length));
}
constexpr strId operator""_sid(const wchar_t* cstr, size_t length) {
	return crc64(reinterpret_cast<const u8*>(cstr), static_cast<u32>(length) * sizeof(wchar_t));
}
constexpr strId operator""_sid(const char16_t* cstr, size_t length) {
	return crc64(reinterpret_cast<const u8*>(cstr), static_cast<u32>(length) * sizeof(char16_t));
}
constexpr strId operator""_sid(const char32_t* cstr, size_t length) {
